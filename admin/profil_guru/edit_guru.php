...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($_FILES['foto']['error'] == 0) {
        ...
        $query = "UPDATE profil_guru SET nama = ?, foto = ?, nip = ?, jabatan = ? WHERE id = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ssssi", $nama, $foto_name, $nip, $jabatan, $id);  // 's' untuk string, 'i' untuk integer
    } else {
        // Update tanpa foto jika tidak ada foto yang diupload
        $query = "UPDATE profil_guru SET nama = ?, nip = ?, jabatan = ? WHERE id = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("sssi", $nama, $nip, $jabatan, $id);  // 's' untuk string, 'i' untuk integer
    }

    // Eksekusi query update
    if ($stmt->execute()) {
        echo "guru berhasil diperbarui!";
        header("Location: guru.php"); // Kembali ke halaman daftar guru setelah update
        exit;
    } else {
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <!-- Page Heading -->
                    <h1 class="h3 mb-2 text-gray-800 font-weight-bold">Tambah guru</h1>
                    <p class="mb-4">Halaman ini digunakan untuk menambah data guru sekolah.</p>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                        <form action="edit_guru.php?id=<?php echo $guru['id']; ?>" method="POST" enctype="multipart/form-data">
                            <div>
                                <label for="nama">Nama guru:</label>
                                <input type="text" class="form-control" name="nama" id="nama" value="<?php echo $guru['nama']; ?>" required>
                            </div>
                            <div>
                                <label for="foto">Foto:</label>
                                <input type="file" class="form-control" name="foto" id="foto">
                                <p>Foto saat ini:<br> <img src="uploads/<?php echo $guru['foto']; ?>" width="200"></p>
                            </div>
                            <div>
                                <label for="nip">NIP:</label>
                                <input type="text" class="form-control" name="nip" id="nip" value="<?php echo $guru['nip']; ?>" required>
                            </div>
                            <div>
                                <label for="jabatan">Jabatan:</label>
                                <textarea name="jabatan" class="form-control" id="jabatan" required><?php echo $guru['jabatan']; ?></textarea>
                            </div>
                            <div>
