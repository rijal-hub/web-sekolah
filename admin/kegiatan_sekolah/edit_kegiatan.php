...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($_FILES['foto']['error'] == 0) {
        ...
        $query = "UPDATE kegiatan_sekolah SET nama_kegiatan = ?, deskripsi = ?, foto = ? WHERE id = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("sssi", $nama_kegiatan, $deskripsi, $foto_name, $id);
    } else {
        // Update tanpa foto jika tidak ada foto yang diupload
        $query = "UPDATE kegiatan_sekolah SET nama_kegiatan = ?, deskripsi = ? WHERE id = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ssi", $nama_kegiatan, $deskripsi, $id);
    }

    // Eksekusi query update
    if ($stmt->execute()) {
        echo "kegiatan berhasil diperbarui!";
        header("Location: kegiatan.php"); // Kembali ke halaman daftar kegiatan setelah update
        exit;
    } else {
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <!-- Page Heading -->
                    <h1 class="h3 mb-2 text-gray-800 font-weight-bold">Edit kegiatan</h1>
                    <p class="mb-4">Halaman ini digunakan untuk mengedit data kegiatan sekolah.</p>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                        <form action="edit_kegiatan.php?id=<?php echo $kegiatan['id']; ?>" method="POST" enctype="multipart/form-data">
                            <div>
                                <label for="nama_kegiatan">Nama kegiatan:</label>
                                <input type="text" class="form-control" name="nama_kegiatan" id="nama_kegiatan" value="<?php echo $kegiatan['nama_kegiatan']; ?>" required>
                            </div><br>
                            <div>
                                <label for="foto">Foto:</label>
                                <input type="file" class="form-control" name="foto" id="foto">
                                <p>Foto saat ini:<br> <img src="uploads/<?php echo $kegiatan['foto']; ?>" width="200"></p>
                            </div>
                            <div>
                                <label for="deskripsi">Deskripsi:</label>
                                <textarea name="deskripsi" class="form-control" id="deskripsi" required><?php echo $kegiatan['deskripsi']; ?></textarea>
                            </div>
                            <div>
